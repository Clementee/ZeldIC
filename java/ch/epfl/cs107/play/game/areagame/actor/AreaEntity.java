...
AreaEntity extends Entity implements Interactable {
    ...
    /**
     * Generic method used by still (unmovable) entities to draw their end-of-life animation
     * and unregister them
     *
     * @param animation  (Animation) the ending animation
     * @param canvas     (Canvas) drawing canvas
     * @param area       (Area) the Area the AreaEntity is in
     * @param areaEntity (AreaEntity) the entity that's being recycled
     */
    protected static void endingAnimationAndUnregister(
            Animation animation, Canvas canvas, Area area, AreaEntity areaEntity) {
        if (!animation.isCompleted()) {
            animation.draw(canvas);
        } else {
            area.unregisterActor(areaEntity);
        }
    }
}
